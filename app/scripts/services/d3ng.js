...
///////goooood
                  ...
                  x   .domain(d3.extent(d3.merge(seriesData)))
                  ...
                  var vertices = d3.merge(data.map(function(line, lineIndex) {
                        return line.values.map(function(point, pointIndex) {
                          var pointKey = line.values + '-' + point['x'];
                          console.log('x', x(point['x']))
                          return [x(point['x']), y(point['y']), lineIndex, pointIndex]; //adding series index to point because data is being flattened
                        })
                      })
                  );
                  ...
                  pointClips
                      .attr('id', function(d, i) { return 'clip-' + id + '-' + d[3] + '-' + d[4] })
            ...
            d3LineWithLegend = function() {
                  ...
                  function chart(selection) {
                    selection.each(function(data) {
                      ...
                      x   .domain(d3.extent(d3.merge(series)))
                          .range([0, width - margin.left - margin.right]);
